...
def get_data(cmd, host=host, apikey=apikey):
    r = requests.get("http://{}/api/v2?apikey={}&cmd={}".format(host, apikey, cmd))
    # print(r.status_code)
...
def get_my_activity(host=host, apikey=apikey, username=username, libraries=libraries):
    ...
    for stream in data["sessions"]:
        if (
            (len(libraries) == 0 or stream["library_name"] in libraries)
            and (username == "" or stream["user"] == username)
            and stream["device"] not in excluded_devices
        ):
